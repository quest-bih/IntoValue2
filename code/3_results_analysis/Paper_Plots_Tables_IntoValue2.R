...
# 4) odds ratio plot
# 5) other calculations for paper
#
#----------------------------------------------------------------------------------------------------------------------
...
#read in dataset
IntoValue_dataset_comb <- read_csv("data/2_dataset_cleaning/final_dataset/iv_main_dataset.csv", na = "NA")
...
write_csv(demo_table_save, "output/demographics_table.csv")
...
write_csv(city_statistics_lead_2y, "output//city_statistics_2years.csv")
...
write_csv(city_statistics_lead_5y, "output/city_statistics_5years.csv")
...
write_csv(city_statistics_comp, "output/city_statistics_comp.csv")
...
ggplot(cum_dist_years, aes(x = months, y = fraction, color = category)) +
  ...
  theme(axis.line = element_line(size = 0.5, linetype = "solid",
                                 colour = "black"))
ggsave("output/Kaplan_Meier.png", width = 33, height = 20, units = "cm", dpi = 300)
...
city_statistics <- read_csv("output/city_statistics_comp.csv")
...
ggsave("output/odds_ratios.png", width = 15, height = 15, units = "cm", dpi = 300)
